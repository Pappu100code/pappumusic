...
class Player extends EventEmitter {
    constructor(pappumusic, node, options) {
        ...
        this.on("playerUpdate", (packet) => {
            ...
            this.pappumusic.emit("playerUpdate", this, packet);
        });
    ...
    async play() {
        if(!this.connected) throw new Error("Player connection is not initiated. Kindly user pappumusic.createConnection() and establish a connection");
        if (!this.queue.length) return;

        this.current = this.queue.shift();

        if (!this.current.track) {
            this.current = await this.current.resolve(this.pappumusic);
        }
        ...
        if(player.previous) {
            if (player.previous.info.sourceName === "youtube") {
                try {
                    ...
                    let response = await this.pappumusic.resolve({ query: data, source: "ytmsearch", requester: player.previous.info.requester });
                ...
                try {
                    scAutoPlay(player.previous.info.uri).then(async (data) => {
                        let response = await this.pappumusic.resolve({ query: data, source: "scsearch", requester: player.previous.info.requester });
                ...
                try {
                    spAutoPlay(player.previous.info.identifier).then(async (data) => {
                        const response = await this.pappumusic.resolve({ query: `https://open.spotify.com/track/${data}`, requester: player.previous.info.requester });
    ...
    connect(options = this) {
        ...
        this.connected = true;
        this.pappumusic.emit("debug", this.guildId, "Player has been connected");
    }
    ...
    destroy() {
        ...
        this.pappumusic.emit("playerDisconnect", this);
        this.pappumusic.emit("debug", this.guildId, "Destroyed the player");

        this.pappumusic.players.delete(this.guildId);
    }
    ...
    trackStart(player, track, payload) {
        this.playing = true;
        this.paused = false;
        this.pappumusic.emit("trackStart", player, track, payload);
    }
    ...
    trackEnd(player, track, payload) {
        ...
        if (this.loop === "track") {
            ...
            this.pappumusic.emit("trackEnd", player, track, payload);
            return player.play();
        }
        ...
        if (player.queue.length === 0) {
            this.playing = false;
            return this.pappumusic.emit("queueEnd", player);
        }
        ...
        this.playing = false;
        this.pappumusic.emit("queueEnd", player);
    }

    trackError(player, track, payload) {
        this.pappumusic.emit("trackError", player, track, payload);
        this.stop();
    }

    trackStuck(player, track, payload) {
        this.pappumusic.emit("trackStuck", player, track, payload);
        this.stop();
    }
    ...
    socketClosed(player, payload) {
        ...
        this.pappumusic.emit("socketClosed", player, payload);
        this.pause(true);
        this.pappumusic.emit("debug", this.guildId, "Player paused, channel deleted, Or Client was kicked");
    }
    ...
    send(data) {
        this.pappumusic.send({ op: 4, d: data });
    }
}
