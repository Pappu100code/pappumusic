...
const { pappumusic } = require("../build/index.js");
...
client.pappumusic = new pappumusic(client, nodes, {
    send: (payload) => {
...
client.on("ready", () => {
    client.pappumusic.init(client.user.id);
...
client.on("messageCreate", async (message) => {
    ...
    if (command === "play") {
        ...
        const player = client.pappumusic.createConnection({
        ...
        const resolve = await client.pappumusic.resolve({ query: query, requester: message.author });
    ...
    if (command === "skip") {
        const player = client.pappumusic.players.get(message.guild.id);
...
client.pappumusic.on("nodeConnect", node => {
    console.log(`Node "${node.name}" connected.`)
})

client.pappumusic.on("nodeError", (node, error) => {
    console.log(`Node "${node.name}" encountered an error: ${error}`)
})

client.pappumusic.on("nodeReconnect", (node) => {
    console.log(`Node "${node.name}" reconnecting.`)
})

client.pappumusic.on("trackStart", async (player, track) => {
    const channel = client.channels.cache.get(player.textChannel);

    channel.send(`Now playing: \`${track.info.title}\` by \`${track.info.author}\`.`);
});

client.pappumusic.on("queueEnd", async (player) => {
    const channel = client.channels.cache.get(player.textChannel);
...
client.on("raw", (d) => {
    ...
    client.pappumusic.updateVoiceState(d);
});
